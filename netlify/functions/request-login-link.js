...
exports.handler = async (event) => {
  connectLambda(event);
  console.log('request-login-link: invoked, method =', event.httpMethod);
  ...
  if (!apiKey) {
    console.error('request-login-link: RESEND_API_KEY is not set in this site\'s environment variables.');
    return { statusCode: 500, body: JSON.stringify({ error: 'Login isn\'t configured yet (missing RESEND_API_KEY).' }) };
  }
  ...
  try {
    ...
    console.error('request-login-link: failed writing token to Blobs:', err.message);
  ...
  try {
    ...
    console.log('request-login-link: Resend responded', res.status, resendBody.slice(0, 500));

    if (!res.ok) {
      return {
        statusCode: 502,
        body: JSON.stringify({
          error: `The login link could not be sent (Resend returned ${res.status}). See the detail below for the actual reason.`,
          detail: resendBody.slice(0, 500)
        })
      };
    }
    ...
    console.error('request-login-link: fetch to Resend threw:', err.message);
