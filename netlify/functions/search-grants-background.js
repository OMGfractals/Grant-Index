...
// Entitlement reads need to see the very latest write, not a cached-for-a-
// moment version — otherwise two searches close together can both read
// "2 used" and both get allowed through past the real limit. Try strong
// consistency first; if this platform context rejects it, fall back rather
// than crash the search entirely.
...
exports.handler = async (event) => {
  ...
  let jobId, topic, degree, residency, age, industry, categories, rawEmail;
  try {
    ({ jobId, topic, degree, residency, age, industry, categories, email: rawEmail } = JSON.parse(event.body || '{}'));
  } catch {
  ...
  //
  // Logged-in users (a verified email came through) are tracked by email
  // instead of IP, so their free-search count and purchased credits follow
  // them across networks and devices instead of resetting whenever their
  // IP changes. Not logged in falls back to the original IP-based key. ----
  ...
  const entitlementKey = email ? `entitlement:email:${email}` : `entitlement:${clientIp}`;
  let entitlement = (await getConsistentRecord('entitlements', entitlementKey))
